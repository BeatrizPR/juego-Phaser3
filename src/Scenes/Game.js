import 'phaser';
import Player from '../Sprites/Player';
import Portal from '../Sprites/Portal';
import Fruits from '../Groups/Fruits';
import Enemies from '../Groups/Enemies';
import Bullets from '../Groups/Bullets';
import config from '../config';

export default class GameScene extends Phaser.Scene {
  constructor (key) {
    super(key);
  }

  init (data) {
    this._LEVEL = data.level;
    this._LEVELS = data.levels;
    this._NEWGAME = data.newGame;
    this.loadingLevel = false;
    if (this._NEWGAME) this.events.emit('newGame');
  }

  create () {
    // redimesionar el juego
    this.events.on('resize', this.resize, this);
    // recibir que teclas se han pulsado
    this.cursors = this.input.keyboard.createCursorKeys();
    this.spaceKey = this.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.SPACE);

    // creo el tilemap
    this.createMap();
    // creo el jugador
    this.createPlayer();
    // creo el portal
    this.createPortal();
    // creo el audio
    this.musicAudio = this.sound.add('music', config);
    this.musicAudio.play();
    this.knifeAudio = this.sound.add('knife');
    this.eatAudio = this.sound.add('eat');
    // creo las frutas
    this.fruits = this.map.createFromObjects('Fruits', 'Fruit', { key: 'fruit' });
    this.fruitsGroup = new Fruits(this.physics.world, this, [], this.fruits);
    // creo los enemigos
    this.enemies = this.map.createFromObjects('Enemies', 'Enemy', {});
    this.enemiesGroup = new Enemies(this.physics.world, this, [], this.enemies);
    // creating las balas que dispara, que son cuchillos
    this.bullets = new Bullets(this.physics.world, this, []);

    // añado las colisiones
    this.addCollisions();

    // actualizar la cámara que sigue al jugador
    this.cameras.main.startFollow(this.player);

    // animaciones del jugador
    this.anims.create({
      key: 'down',
      frames: this.anims.generateFrameNumbers('characters', { start: 19, end: 26 }),
      frameRate: 10
    });

    this.anims.create({
      key: 'up',
      frames: this.anims.generateFrameNumbers('characters', { start: 1, end: 8 }),
      frameRate: 10
    });

    this.anims.create({
      key: 'right',
      frames: this.anims.generateFrameNumbers('characters', { start: 28 , end: 36 }),
      frameRate: 10
    });
    this.anims.create({
      key: 'left',
      frames: this.anims.generateFrameNumbers('characters', { start: 10, end: 17 }),
      frameRate: 10
    });

      

  }

  update () {
    this.player.update(this.cursors);
    // compruebo que se pulsa el espacio
    if (Phaser.Input.Keyboard.JustDown(this.spaceKey)) {
      this.bullets.fireBullet(this.player.x, this.player.y, this.player.direction);
    }
  }

  addCollisions () {
    // incluyo las colisiones
    this.physics.add.collider(this.player, this.blockedLayer);
    this.physics.add.collider(this.enemiesGroup, this.blockedLayer);
    this.physics.add.overlap(this.player, this.enemiesGroup, this.player.enemyCollision.bind(this.player));
    this.physics.add.overlap(this.player, this.portal, this.loadNextLevel.bind(this, false));
    this.physics.add.overlap(this.fruitsGroup, this.player, this.fruitsGroup.collectFruit.bind(this.fruitsGroup));
    this.physics.add.overlap(this.bullets, this.enemiesGroup, this.bullets.enemyCollision);
  }

  createPlayer () {
    this.map.findObject('Player', (obj) => {
      if (this._NEWGAME && this._LEVEL === 1) {
        if (obj.type === 'StartingPosition') {
          this.player = new Player(this, obj.x, obj.y);
        }
      } else {
        this.player = new Player(this, obj.x, obj.y);
      }
    });
  }

  createPortal () {
    this.map.findObject('Portal', (obj) => {
      if (this._LEVEL === 1) {
        this.portal = new Portal(this, obj.x, obj.y - 68);
      } else if (this._LEVEL === 2) {
        this.portal = new Portal(this, obj.x, obj.y + 70);
      }
    });
  }

  resize (width, height) {
    if (width === undefined) {
      width = this.sys.game.config.width;
    }
    if (height === undefined) {
      height = this.sys.game.config.height;
    }
    this.cameras.resize(width, height);
  }

  createMap () {
    // incluir agua en el fondo
    //this.add.tileSprite(0, 0, 8000, 8000, 'RPGpack_sheet', 31);
    // create the tilemap
    this.map = this.make.tilemap({ key: this._LEVELS[this._LEVEL] });
    // incluyo la imagen tileset 
    this.tiles = this.map.addTilesetImage('RPGpack_sheet');
    // creo las capas
    this.backgroundLayer = this.map.createStaticLayer('Background', this.tiles, 0, 0);
    this.blockedLayer = this.map.createStaticLayer('Blocked', this.tiles, 0, 0);
    this.blockedLayer.setCollisionByExclusion([-1]);
  }

  loadNextLevel (endGame) {
    if (!this.loadingLevel) {
      this.cameras.main.fade(500, 0, 0, 0);
      this.cameras.main.on('camerafadeoutcomplete', () => {
        if (endGame) {
          this.scene.restart({ level: 1, levels: this._LEVELS, newGame: true });
        } else if (this._LEVEL === 1) {
          this.scene.restart({ level: 2, levels: this._LEVELS, newGame: false });
        } else if (this._LEVEL === 2) {
          this.scene.restart({ level: 1, levels: this._LEVELS, newGame: false });
        }
      });
      this.loadingLevel = true;
    }
  }
};
